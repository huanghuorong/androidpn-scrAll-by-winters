...
public class PersistentConnectionListener implements ConnectionListener {
    ...
    public void connectionClosed() {
        Log.d(LOGTAG, "Monitor connectionClosed event.");
    }

    @Override
    public void connectionClosedOnError(Exception e) {
        Log.d(LOGTAG, "Monitor connectionClosedOnError event.");
        if (xmppManager.getConnection() != null
                && xmppManager.getConnection().isConnected()) {
        	 AndroidpnLogger.i(LOGTAG, "connectionClosedOnError,now call fun disconnect()");
        	 // 这里为什么不调用 xmppManager.disConnect()? 
            xmppManager.getConnection().disconnect();
        }
        AndroidpnLogger.i(LOGTAG, "connectionClosedOnError,now call fun startReconnectionThread");
