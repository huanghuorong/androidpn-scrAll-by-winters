...
public class PushDemoService extends Service {
    ...
    private static final String LOGTAG = LogUtil
            .makeLogTag(PushDemoService.class);

    public static final String SERVICE_NAME = PushDemoService.class.getName();
    ...
    private BroadcastReceiver heartActionBroadCastReceiver;
    ...
    public PushDemoService() {
        notificationReceiver = new PushDemoReceiver();
        connectivityReceiver = new ConnectivityReceiver(this);
        //phoneStateListener = new PhoneStateChangeListener(this);
        heartActionBroadCastReceiver = new HeartActionBroadCastReceiver(this);
    ...
    public void connect() {
        ...
        taskSubmitter.submit(new Runnable() {
            public void run() {
                PushDemoService.this.getXmppManager().connect();
            }
        });
    }
    ...
    public void disconnect() {
        ...
        taskSubmitter.submit(new Runnable() {
            public void run() {
                PushDemoService.this.getXmppManager().disconnect();
            }
        });
    }
    ...
    private void registerHeartActionBroadCastReceiver() {
    	AndroidpnLogger.d(LOGTAG, "registerHeartActionBroadCastReceiver...");
    	 // Schedule the alarm!
    	
    	Intent intent = new Intent();  
        intent.setAction(Constants.ACTION_ALARM_NOTIFICATION);  //过滤器 aliusa.cn.MyService（闹钟也需要过滤器）
        pi = PendingIntent.getBroadcast(this, 0, intent, Intent.FLAG_ACTIVITY_NEW_TASK);    ;          
        
        long now = System.currentTimeMillis();    
   	 	int interval = SmackConfiguration.getKeepAliveInterval();
   	    AndroidpnLogger.d(LOGTAG, "Schedule a clock in "+interval+"ms, delay "+interval+"ms");
   	    alarmManager = (AlarmManager)getSystemService(ALARM_SERVICE);   
   	 	alarmManager.setInexactRepeating(AlarmManager.RTC_WAKEUP, now+interval, interval, pi);  
        
    	 // 注册侦听
    	 IntentFilter filter = new IntentFilter();
    	 filter.addAction(Constants.ACTION_ALARM_NOTIFICATION);
         registerReceiver(heartActionBroadCastReceiver, filter);
	}
    
    private void unregisterHeartActionBroadCastReceiver() {
    	 Log.d(LOGTAG, "unregisterHeartActionBroadCastReceiver...");
    	// 反注册 AlarmManager
    	 //if(alarmManager!=null)alarmManager = (AlarmManager)getSystemService(ALARM_SERVICE);   
    	 if(alarmManager!=null && pi!=null)alarmManager.cancel(pi);
    	 // 反注册侦听
    	 IntentFilter filter = new IntentFilter();
    	 unregisterReceiver(heartActionBroadCastReceiver);
	}
    ...
    public class TaskSubmitter {
        ...
        final PushDemoService notificationService;

        public TaskSubmitter(PushDemoService notificationService) {
    ...
    public class TaskTracker {
        ...
        public TaskTracker(PushDemoService notificationService) {
            this.notificationService = notificationService;
            this.count = 0;
        }
