...
public class XMPPConnection extends Connection {
    ...
    @Override
    public synchronized Packet keepHeartbeat() throws XMPPException {
        if (!isConnected()) {
            throw new XMPPException("Not connected to server.");
        }
        if (!authenticated) {
            throw new XMPPException("Has not logged yet.");
        }

        Presence presence = new Presence(Presence.Type.available);
        PacketCollector collector = createPacketCollector(new PacketIDFilter(presence.getPacketID()));
        // Send the packet
        sendPacket(presence);
        // Wait up to a certain number of seconds for a response from the server.
        // 解包有客户端处理。
       return collector.nextResult(SmackConfiguration.getPacketReplyTimeout());
    }
    ...
    private void initConnection() throws XMPPException {
        ...
        try {
            ...
            // by winters ,由客户端通过检测时钟 AlarmManager 来保持心跳，smack原来的发送空内容心跳有问题，把路由wan拔了也检查不到发送异常，这里是基于TCP可靠连接而不是UDP
            // 2012.11.27 还是使用此心跳，节省流量，发送包会导致每次的请求包大小大185*2 byte
