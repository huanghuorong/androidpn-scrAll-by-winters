...
class PacketWriter {
    ...
    */ 
    ...
    void startKeepAliveProcess() {
        ...
        AndroidpnLogger.i("PacketWriter", "Scheme a KeepAliveTask,keepAliveInterval = "+keepAliveInterval);
    ...
    private void writePackets(Thread thisThread) {
        try {
            ...
            while (!done && (writerThread == thisThread)) {
                ...
                if (packet != null) {
                    synchronized (writer) {
                    	AndroidpnLogger.i("writePackets", "writePackets 001: "+packet.toXML()+",length ["+packet.toXML().length()+"]byte");
            ...
            try {
                synchronized (writer) {
                   while (!queue.isEmpty()) {
                       ...
                       AndroidpnLogger.i("writePackets(Thread thisThread) ", "writePackets 002: "+packet.toXML()+",length ["+packet.toXML().length()+"]byte");
                        writer.write(packet.toXML());
                    }
                    writer.flush();
                }
            ...
            while (!done && keepAliveThread == thread) {
                synchronized (writer) {
                    ...
                    if (System.currentTimeMillis() - lastActive >= delay) {
                        try {
                        	// Android设备CPU进入休眠状态，线程会被挂起，需要使用AlarmManager进行唤醒
                        	// KdweiboLogger.i("PacketWriter", "Write to server at "+(new Date().toString()) );
                            writer.write(" ");
                            writer.flush(); 
                        }catch (Exception e) {
                            // 写入异常说明已经和服务器断开，此时需要通知重连接
                        }
                    }
                }
                try {
